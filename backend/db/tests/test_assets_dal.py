...
from backend.db.dal import AssetsDAL
from backend.db.dal.schemas import AssetsCreate
from backend.db.data_models import Assets
...
async def test_create_asset_with_metadata(db_session: AsyncSession) -> None:
    ...
    # Act
    result: Assets = await AssetsDAL.create(db_session, asset_create)
    ...
    db_obj = await db_session.get(Assets, result.id)
